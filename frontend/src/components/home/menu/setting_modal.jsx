        ...
        <div className="fixed top-0 left-0 w-screen h-screen bg-black bg-opacity-60 z-[9999888] flex justify-center items-center pointer-events-auto">
            <div className="relative overflow-hidden"> {/* 환경설정 전체 이미지 */}
                ...
                <div className="absolute top-10 left-1/2 -translate-x-1/2 z-[1000] text-[#455970] text-[30px] tracking-widest font-bold">
                    ...
                    {/* 음량 조절, 주기 설정 */}
                    <div className="absolute flex flex-row p-5 top-[70px] bg-[#FFFCFA] w-[400px] h-[150px] left-1/2 -translate-x-1/2 z-[1000] rounded-xl">
                        <div className='flex flex-col text-[20px] w-[210px] font-semibold'>

                            <div className="flex flex-row w-[300px] pb-5">
                                <div className='pr-2'>배경음</div>

                                <div onClick={() => setToggledSound(prev => !prev)}>
                                    <div className={`w-[90px] h-[40px] bg-gray-300 rounded-full flex items-center pl-2 cursor-pointer duration-200
                                                ${toggledSound ? "bg-[#684544] pl-11" : "bg-gray-300"}`} >
                                        <div className={`w-[35px] h-[35px] rounded-full cursor-pointer transition-colors
                                                            ${toggledSound ? "bg-white" : "bg-white"}`}
                                                            ></div>
                                    </div>
                                </div>
                            ...
                            <div className='flex flex-row  w-[300px]'>
                                <div className='pr-2'>푸시알람</div>

                                <div onClick={() => setToggledAlarm(prev => !prev)}>
                                    <div className={`w-[90px] h-[40px] bg-gray-300 rounded-full flex items-center pl-2 cursor-pointer duration-200
                                        ${toggledAlarm ? "bg-[#684544] pl-11" : "bg-gray-300"}`} >
                                        <div className={`w-[35px] h-[35px] rounded-full cursor-pointer transition-colors
                                                ${toggledAlarm ? "bg-white" : "bg-white"}`}
                                                ></div>
                                    </div>
                                 </div>
                            </div>
                        ...
                        <div className='flex flex-row'>
                            ...
                            <div className='text-[10px]'>드롭박스</div>
                        </div>              
                    </div>

                    {/* 간단한 스토리 설명 */}
                    <div className="absolute top-[240px] bg-[#FFFCFA] w-[400px] h-[300px] left-1/2 -translate-x-1/2 z-[1000] rounded-xl">
