...
def check_all_base_conditions(feat_df: pd.DataFrame, thresholds: Dict, calibration: Dict = None) -> np.ndarray:
    ...
    for k, v in thresholds.items():
        ...
        if k.startswith('calibration_'):
            if not isinstance(v, dict) or not calibration:
                if calibration is None:
                    print(f"  Skipping calibration threshold {k} (no calibration data)")
                else:
            ...
            if feat_name in feat_df.columns:
                ...
                print(f"  {feat_name} {operator} {threshold_value:.4f} (calib:{baseline_value:.4f}{offset:+.4f}): {passes}/{len(feat_df)} frames pass ({passes/len(feat_df)*100:.1f}%)")
            else:
